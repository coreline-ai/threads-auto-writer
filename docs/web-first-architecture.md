...
| 구성             | 패키지명                       | 역할                                                    |
| ---------------- | ------------------------------ | ------------------------------------------------------- |
| 자체 웹          | `@threadflow-os/web`           | 설치 전 1턴 생성·편집·승인·보관·클립보드 전달           |
...
| 로컬 Gateway     | `@threadflow-os/codex-gateway` | Codex OAuth Provider Proxy, 생성 SSE, loopback 보안     |
...
2. `pnpm start:web`으로 웹과 Gateway를 동시에 실행한다.
3. `http://127.0.0.1:4173`의 **설정**에서 `.threadflow/session-secret` 파일 내용을 입력한다.
4. **ChatGPT 구독으로 로그인**을 눌러 Codex OAuth를 완료한다.
5. 웹에서 1턴 작성 품질과 보관 흐름을 먼저 사용한다.
