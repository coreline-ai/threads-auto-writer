export const GENERATION_FINGERPRINT_VERSION =
  "threadflow-generation-fingerprint/v1";
...
function normalizeCanonical(value: unknown): unknown {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new TypeError(
        "Fingerprint input must not contain a non-finite number.",
      );
    return value;
  }
  ...
  throw new TypeError(
    `Fingerprint input contains an unsupported ${typeof value} value.`,
  );
}
...
export function sha256BytesHex(input: Uint8Array | ArrayBuffer): string {
  const bytes =
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first =
        (h + bigSigma1 + choice + SHA256_CONSTANTS[index]! + words[index]!) >>>
        0;
      const bigSigma0 =
...
export function generationRequestFingerprint(request: unknown): string {
  return sha256Hex(
    canonicalJson({
